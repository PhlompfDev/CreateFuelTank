...
public class FuelTankGameTests {
    ...
    public static void multiblock_feeds_from_any_part(GameTestHelper helper) {
        ...
        for (int x = 1; x <= 2; x++)
            for (int z = 1; z <= 2; z++)
                helper.setBlock(new BlockPos(x, 1, z), com.createvehiclesurplus.VehicleSurplusBlocks.FUEL_TANK.getDefaultState());

        // The multiblock forms on the first tick; fill the controller once it exists.
        helper.runAfterDelay(5, () -> {
            FluidTankBlockEntity part = ConnectivityHandler.partAt(VehicleSurplusBlockEntities.FUEL_TANK.get(), helper.getLevel(), helper.absolutePos(new BlockPos(2, 1, 2)));
    ...
    private static void placeEngineAndTank(GameTestHelper helper, net.minecraft.world.level.material.Fluid fluid, int amount) {
        helper.setBlock(ENGINE, CDGBlocks.DIESEL_ENGINE.getDefaultState());
        helper.setBlock(TANK, com.createvehiclesurplus.VehicleSurplusBlocks.FUEL_TANK.getDefaultState());
