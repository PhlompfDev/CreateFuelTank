...
public class LongFuelTankGameTests {
    ...
    public static void rows_on_different_axes_do_not_merge(GameTestHelper helper) {
        ...
        helper.setBlock(new BlockPos(3, 1, 1), FuelTankBlocks.LONG_FUEL_TANK.getDefaultState().setValue(LongFuelTankBlock.AXIS, Axis.Z));
        helper.setBlock(new BlockPos(3, 1, 2), FuelTankBlocks.LONG_FUEL_TANK.getDefaultState().setValue(LongFuelTankBlock.AXIS, Axis.Z));
    ...
    private static void placeRow(GameTestHelper helper, Axis axis, int from, int count) {
        for (int i = 0; i < count; i++) {
            ...
            helper.setBlock(pos, FuelTankBlocks.LONG_FUEL_TANK.getDefaultState().setValue(LongFuelTankBlock.AXIS, axis));
        }
    }
    ...
    private static LongFuelTankBlockEntity controllerOf(GameTestHelper helper, BlockPos part) {
        FluidTankBlockEntity be = ConnectivityHandler.partAt(FuelTankBlockEntities.LONG_FUEL_TANK.get(), helper.getLevel(), helper.absolutePos(part));
        if (be == null)
